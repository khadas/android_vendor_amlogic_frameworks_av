...
TSPacker::TSPacker(int width, int height, bool hasAudio) :
    mWidth(width),
    mHeight(height),
    mHasAudio(hasAudio),
    mPrevTimeUs(-1ll),
...
int TSPacker::threadFunc()
{
    ...
    while (mStarted == true) {
        ...
        ALOGV("[%s %d] csd buffer size:%d", __FUNCTION__, __LINE__, tESBuffer->range_length());
...
status_t TSPacker::start(MetaData *params)
{
    ...
    params_video->setInt32(kKeyFrameRate, 30);
    params_video->setInt32(kKeyBitRate, 2000000);

    mVideoConvertor = new ESConvertor(0);
    ...
    if (mHasAudio) {
        ...
        mIsPcmAudio = 0;
        mAudioConvertor = new ESConvertor(1);
...
void TSPacker::headFinalize() {
...

    {
        ...
        ALOGV("[%s %d] make AVC video descriptor, size is 6",__FUNCTION__, __LINE__);
...

    {
        ...
        // reserved = 11111b
        data[3] = 0x1f;
        ALOGV("[%s %d] make AVC timing and HRD descriptor, size is 4",__FUNCTION__, __LINE__);
        mDescriptors.push_back(descriptor);
    }
    ...
    if (mHasAudio && mIsPcmAudio) {
        ...
        ALOGV("[%s %d] make  LPCM audio stream descriptor, size is 4",__FUNCTION__, __LINE__);
...
status_t TSPacker::read( MediaBuffer **buffer,
				 const ReadOptions *options)
{
    ...
    *buffer = NULL;
    ...
    //ALOGV("[%s %d] aBuffer:%x aBuffer->size:%d tBuffer:%x", __FUNCTION__, __LINE__, aBuffer->data(), aBuffer->size(), tBuffer->data());
...
void TSPacker::signalBufferReturned(MediaBuffer *buffer) {
