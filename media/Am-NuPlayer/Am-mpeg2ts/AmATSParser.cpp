...
AmATSParser::SyncEvent::SyncEvent(off64_t offset)
    : mHasReturnedData(false), mOffset(offset), mTimeUs(0) {}

void AmATSParser::SyncEvent::init(off64_t offset, const sp<MediaSource> &source,
        int64_t timeUs) {
...
void AmATSParser::SyncEvent::reset() {
...
bool AmATSParser::Program::switchPIDs(const Vector<StreamInfo> &infos) {
    bool success = false;
    ...
    for (size_t i = 0; i < infos.size(); ++i) {
        ...
        if (index < 0) {
            ...
            if (((stream->isAudio() || info.mType == 0x06) && AmATSParser::mNoAudio)
                ...
                ALOGI("ignore %s stream(%d)!", stream->isAudio() ? "audio" : "video", info.mType);
                stream.clear();
                continue;
            }
...
int64_t AmATSParser::Program::recoverPTS(uint64_t PTS_33bit) {
...
#if 0   //  this has a modif bause h265`s vps
...
status_t AmATSParser::Stream::parsePES(ABitReader *br, SyncEvent *event) {
...
status_t AmATSParser::Stream::flush(SyncEvent *event) {
...
status_t AmATSParser::parseAdaptationField(ABitReader *br, unsigned PID) {
    ...
    if (adaptation_field_length > 0) {
        if (adaptation_field_length * 8 > br->numBitsLeft()) {
            ALOGI("Adaptation field should be included in a single TS packet.");
            return ERROR_MALFORMED;
        }
...
status_t AmATSParser::parseTS(ABitReader *br, SyncEvent *event) {
    ALOGV("---");
...
uint32_t AmATSParser::PSISection::CRC_TABLE[] = {
...
void AmATSParser::PSISection::setSkipBytes(uint8_t skip) {
    mSkipBytes = skip;
}
...
bool AmATSParser::PSISection::isCRCOkay() const {
    if (!isComplete()) {
        return false;
    }
