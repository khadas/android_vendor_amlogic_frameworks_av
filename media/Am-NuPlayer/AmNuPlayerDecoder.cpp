...
sp<AMessage> AmNuPlayer::Decoder::getStats() const {
...
status_t AmNuPlayer::Decoder::setVideoSurface(const sp<Surface> &surface) {
...
void AmNuPlayer::Decoder::onSetParameters(const sp<AMessage> &params) {
    if (mCodec == NULL) {
...
void AmNuPlayer::Decoder::doFlush(bool notifyComplete) {
    if (mCCDecoder != NULL) {
        mCCDecoder->flush();
    }
...
void AmNuPlayer::Decoder::onFlush() {
    doFlush(true);
 ...
 */
bool AmNuPlayer::Decoder::doRequestBuffers() {
...
void AmNuPlayer::Decoder::handleError(int32_t err)
{
...
bool AmNuPlayer::Decoder::handleAnInputBuffer(size_t index) {
    if (isDiscontinuityPending()) {
        return false;
    }
...
void AmNuPlayer::Decoder::handleOutputFormatChange(const sp<AMessage> &format) {
    if (!mIsAudio) {
        int32_t width, height;
...
bool AmNuPlayer::Decoder::isDiscontinuityPending() const {
    return mFormatChangePending || mTimeChangePending;
}

void AmNuPlayer::Decoder::finishHandleDiscontinuity(bool flushOnTimeChange) {
    ...
    // Notify AmNuPlayer to either shutdown decoder, or rescan sources
