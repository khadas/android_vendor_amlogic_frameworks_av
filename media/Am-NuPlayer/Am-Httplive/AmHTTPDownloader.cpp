...
//#define LOG_NDEBUG 0
#define LOG_TAG "AmHTTPDownloader"
#include <utils/Log.h>
...
AmHTTPDownloader::AmHTTPDownloader(
...
void AmHTTPDownloader::reconnect() {
    AutoMutex _l(mLock);
    mDisconnecting = false;
}

void AmHTTPDownloader::disconnect() {
    {
        AutoMutex _l(mLock);
        mDisconnecting = true;
    }
...
bool AmHTTPDownloader::isDisconnecting() {
    AutoMutex _l(mLock);
 ...
 *
 */
ssize_t AmHTTPDownloader::fetchBlock(
    ...
    if (isPlaylist && size > 10 * 1024 * 1024 ) {
        ALOGE("fetchBlock  file big no m3u8");
        return  ERROR_UNSUPPORTED;
    }
...
ssize_t AmHTTPDownloader::fetchFile(
        const char *url, sp<ABuffer> *out, String8 *actualUrl,bool isPlaylist) {
    ssize_t err = fetchBlock(url, out, 0, -1, 0, actualUrl, true /* reconnect */,isPlaylist);
...
sp<AmM3UParser> AmHTTPDownloader::fetchPlaylist(
    ...
    *unchanged = false;
    ...
    sp<ABuffer> buffer;
    String8 actualUrl;
    ssize_t err = fetchFile(url, &buffer, &actualUrl,true);
