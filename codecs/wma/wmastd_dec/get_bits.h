...
static INLINE unsigned int get_bits(GetBitContext *s, int n){
    ...
    OPEN_READER(re, s)        //unsigned int re_index= (gb)->index;
                              //int      re_cache= 0;
    UPDATE_CACHE(re, s)       //         re_cache= AV_RB32( ((const uint8_t*)(gb)->buffer)+(re_index>>3) ) << (re_index&0x07);
    tmp= SHOW_UBITS(re, s, n);//         temp=((( int32_t)(re_cache))>>(32-(n)))
    LAST_SKIP_BITS(re, s, n)  //         re_index+=n;
    CLOSE_READER(re, s)       //         (gb)->index=re_index;
