        ...
        if ((history < 128) && (i + 1 < nb_samples)) {
            ...
            if (block_size > 0) {
                if (block_size >= nb_samples - i) {
                    ALOGI("invalid zero block size of %d %d %d\n", block_size,nb_samples, i);
                          ...
                          int channels)
{
    ...
    if (!output_samples || output_samples > alac->max_samples_per_frame) {
        ALOGI("invalid samples per frame: %d\n",output_samples);
    ...
    if (!alac->nb_samples) {
        ...
        if (0/*(ret = avctx->get_buffer(avctx, &alac->frame)) < 0*/) {
            ALOGI("get_buffer() failed\n");
            return ret;
        }
    } else if (output_samples != alac->nb_samples) {
        ALOGE("sample count mismatch: %u != %d\n",output_samples, alac->nb_samples);
        return AVERROR_INVALIDDATA;
    }
    ...
    if (is_compressed) {
        ...
        for (ch = 0; ch < channels; ch++) {
            ...
            if (prediction_type[ch] == 15) {
                ...
                ALOGI("unknown prediction type: %i\n", prediction_type[ch]);
            }
            lpc_prediction(alac->predict_error_buffer[ch],
...
int alac_decode_frame(AVCodecContext *avctx, void *data,
                             int *got_frame_ptr, AVPacket *avpkt)
{
	...
	alac->frame.data[0] = (uint8_t*)data;
	if(alac->frame.data[0]==NULL)
	{
       ALOGE("malloc space for pcm failed\n");
	}
    alac->frame.data[1] =alac->frame.data[0]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[2] =alac->frame.data[1]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[3] =alac->frame.data[2]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[4] =alac->frame.data[3]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[5] =alac->frame.data[4]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[6] =alac->frame.data[5]+ alac->max_samples_per_frame*sizeof(int);
	alac->frame.data[7] =alac->frame.data[6]+ alac->max_samples_per_frame*sizeof(int);
    ...
    while (get_bits_left(&alac->gb) >= 3) {
        ...
        if (element > TYPE_CPE && element != TYPE_LFE) {
            ALOGE("syntax element unsupported: %d\n", element);
            return AVERROR_PATCHWELCOME;
        }

        channels = (element == TYPE_CPE) ? 2 : 1;
        if (ch + channels > alac->channels) {
            ALOGE("invalid element channel count\n");
            return AVERROR_INVALIDDATA;
        }
    ...
    if (!got_end) {
        ALOGE("no end tag found. incomplete packet.\n");
        return AVERROR_INVALIDDATA;
    }

    if (avpkt->size * 8 - get_bits_count(&alac->gb) > 8) {
       ALOGE("Error : %d bits left\n", avpkt->size * 8 - get_bits_count(&alac->gb));
    }
...
av_cold int alac_decode_close(AVCodecContext *avctx)
{
    ALACContext *alac = avctx->priv_data;
...
static int alac_set_info(ALACContext *alac)
{
    ...
    if (!alac->max_samples_per_frame || alac->max_samples_per_frame > INT_MAX) {
        ALOGI("max samples per frame invalid: %u\n",
...
void av_log_ask_for_sample(void *avc, const char *msg, ...)
{
    ...
    //if (msg)
    //   LOGI(msg, argument_list);
     ALOGW("If you want to help, upload a sample "
...
av_cold int alac_decode_init(AVCodecContext * avctx)
{
    int ret;
    int req_packed;
    ALACContext *alac = avctx->priv_data;
    ...
    if (alac->avctx->extradata_size != ALAC_EXTRADATA_SIZE) {
        ALOGE("expected %d extradata bytes\n",ALAC_EXTRADATA_SIZE);
        return -1;
    }
    if (alac_set_info(alac)) {
        ALOGE("set_info failed\n");
        return -1;
    }
    ...
    default: av_log_ask_for_sample(avctx, "Sample depth %d is not supported.\n",alac->sample_size);
	         ALOGE("Sample depth %d is not supported.\n",alac->sample_size);
             return AVERROR_PATCHWELCOME;
    }
    ...
    if (avctx->channels > MAX_CHANNELS) {
        ALOGE("Unsupported channel count: %d\n",avctx->channels);
        return AVERROR_PATCHWELCOME;
    }
    ...
    ALOGI("ALAC_Audio_INFO: samplerate/%d channels/%d sample_size/%d \n",avctx->sample_rate, avctx->channels, alac->sample_size);
    return 0;
}
