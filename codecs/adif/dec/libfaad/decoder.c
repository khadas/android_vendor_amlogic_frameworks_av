...
long NEAACDECAPI NeAACDecInit_EnableParseOnly(NeAACDecHandle hpDecoder)
{
    NeAACDecStruct* hDecoder = (NeAACDecStruct*)hpDecoder;
    hDecoder->ParseOnlyFlag=1;
    return 0;
}
...
//#endif
        ...
        }else if(buffer[0]=='F' && buffer[1]=='M' && buffer[2]=='P'&& buffer[3]=='4'){
             NeAACDecInit2(hDecoder, buffer+4, buffer_size-4 , samplerate, channels);
			 ALOGI("[%s %d] samplerate/%d channels/%d",__FUNCTION__,__LINE__,*samplerate,*channels);
        }
...
unsigned long NEAACDECAPI GetAACFrames(NeAACDecHandle hpDecoder,
                                 unsigned char *buffer,
                                 unsigned long buffer_size)
{
    ...
    {	
        offset = get_latm_frame(&hDecoder->latm_config, &ld);
        if(offset == -1U)
        {
        	return -1;
        }
...
//#endif
    ...
    if ((channels == 5 || channels == 6) && hDecoder->config.downMatrix && !hDecoder->ParseOnlyFlag)
    {
...
//#endif
    ...
    if (((hDecoder->sample_buffer == NULL) ||(hDecoder->alloced_channels != output_channels))&& !hDecoder->ParseOnlyFlag)
    {
...
#ifdef SBR_DEC
    if (((hDecoder->sbr_present_flag == 1) || (hDecoder->forceUpSampling == 1)) && !hDecoder->ParseOnlyFlag)
    {
        uint8_t ele;
